 ...
 *   `<md-card>` would be given a class `md-card`.  This is done so that material2 can style 
...
{
  constructor(a_renderer: Renderer, a_elementRef: ElementRef)
  {
    const element = a_elementRef.nativeElement as Node;
    a_renderer.setElementClass(element, 'md2', true);
    a_renderer.setElementClass(element, element.nodeName.toLowerCase(), true);
...
} // class StyleCompatibilityModule
